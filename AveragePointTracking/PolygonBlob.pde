...
class PolygonBlob extends Polygon {
  ...
  void createPolygon(BlobDetection theBlobDetection, float kinectWidth, float kinectHeight) {
      ...
      if (npoints > 0) {
        ...
        for (int i=0; i<contours.size(); i++) {
          ...
          if (fp.y > kinectHeight-5 && fp.x < closestPoint.x) { 
            ...
            closestPoint = fp; // set this as the selected contour 
            selectedContour = i; // set selectedPoint to 0 (which signals first point) 
            selectedPoint = 0;
