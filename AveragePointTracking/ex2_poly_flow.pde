...
int totalParticles = 2250;
float particleThickness = 2.5;
...
NoisyParticle[] flow = new NoisyParticle[totalParticles];

void ex2_poly_flow_setup() {
  setupBlobPoly();
  
  // set stroke weight (for particle display) to 2.5
  strokeWeight(particleThickness);
...
void setRandomColors(int nthFrame) {
  if (frameCount % nthFrame == 0) {
    ...
    int[] paletteStrings = palettes[int(random(palettes.length))];
    // turn strings into colors
    color[] colorPalette = new color[paletteStrings.length];
    for (int i=0; i<paletteStrings.length; i++) {
      colorPalette[i] = paletteStrings[i];
    }
